...
export class VideoAnalyzer {
  ...
  constructor(
    ...
    this.currentStatus$.subscribe(status => {
      this.ctx?.fillText(status.toString(), 0, 0);
    });
    this.smoothedStatus$.subscribe(status => {
      this.ctx?.fillText(status.toString(), 0, 60);
    });
  }
  ...
  private async setupCamera(video: HTMLVideoElement, mobile: boolean) {
